...
|-------|--------------|----------------|---------|-------------------|
| `Microsoft.AspNetCore.Authentication.JwtBearer` | 10.0.11 | 10.0.xx (en son) | Güvenlik düzeltmeleri ve JWT doğrulama iyileştirmeleri | OIDC akışlarında performans artışı, Keycloak uyumluluğu güçlendirildi |
| `Microsoft.AspNetCore.OpenApi` | 10.0.11 | 10.0.xx (en son) | OpenAPI spesifikasyonu güncellemeleri | OpenAPI 3.1 desteği iyileştirildi |
| `Microsoft.EntityFrameworkCore.*` | 10.0.11 | 10.0.xx (en son) | LINQ çeviri iyileştirmeleri ve performans düzeltmeleri | `DateTimeOffset` karşılaştırmaları için SQL çevirisi güçlendirildi |
| `Npgsql.EntityFrameworkCore.PostgreSQL` | 10.0.3 | 10.0.xx (en son) | PostgreSQL 18 uyumluluk iyileştirmeleri | Yeni PostgreSQL 18 özellikleri desteği |
...
| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `ClickHouse.Driver` | 1.3.0 | 1.4.x (mevcut en son) | ClickHouse 26.7 uyumluluğu ve performans iyileştirmeleri | Yeni veri tipleri desteği, bağlantı havuzu optimizasyonları |
| `AWSSDK.S3` | 4.0.102.1 | 4.x (en son) | RustFS S3 API uyumluluğu | Yeni S3 API özellikleri, hata yönetimi iyileştirmeleri |
| `SSH.NET` | 2026.0.0 | 2026.xx (en son) | Cihaz config toplama güvenliği | SSH protokol güncellemeleri, anahtar değişim algoritmaları |
| `Google.Protobuf` | 3.35.1 | 3.xx (en son) | OTLP mesaj çözme performansı | Protobuf 3 yeni özellikler |
| `Grpc.Tools` | 2.83.0 | 2.xx (en son) | OTLP protokol güncellemeleri | gRPC performans iyileştirmeleri |

### 1.3 İsteğe Bağlı Güncellemeler

| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `YamlDotNet` | 18.1.0 | 18.x (en son) | Parser YAML dosyaları için minor düzeltmeler | YAML 1.2 uyumluluk iyileştirmeleri |
| `ZstdSharp.Port` | 0.8.8 | 0.x (en son) | Sıkıştırma performansı | Yeni sıkıştırma seviyeleri |
| `System.CommandLine` | 2.0.11 | 2.x (en son) | CLI araçları için iyileştirmeler | Tab completion desteği |

### 1.4 Test Kütüphaneleri

| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `xunit.v3` | 3.2.2 | 3.x (en son) | Test altyapısı güncellemeleri | Yeni assertion'lar, parallel testing iyileştirmeleri |
| `Testcontainers` | 4.14.0 | 4.x (en son) | Konteyner tabanlı testler | Yeni modüller, yaşam döngüsü yönetimi |
| `NetArchTest.Rules` | 1.3.2 | 1.x (en son) | Mimari test kuralları | Yeni kural türleri |
...
| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `next` | ^15.5.4 | ^15.x (en son) | **Güvenlik düzeltmeleri kritik** | Server-Side Request Forgery (SSRF) yamaları, middleware güvenlik iyileştirmeleri |
| `react` | ^19.1.1 | ^19.x (en son) | React 19 kararlı sürüm iyileştirmeleri | Concurrent rendering, Server Components optimizasyonları |
| `react-dom` | ^19.1.1 | ^19.x (en son) | React DOM güvenlik güncellemeleri | XSS koruma iyileştirmeleri |
| `jose` | ^6.0.11 | ^6.x (en son) | JWT/OIDC güvenlik yamaları | Keycloak token doğrulama uyumluluğu |
...
| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `redis` | ^6.2.1 | ^6.x (en son) | Redis 8 uyumluluğu ve performans | Yeni komutlar, connection pooling iyileştirmeleri |
| `typescript` | ^5.8.3 | ^5.x (en son) | Tip güvenliği iyileştirmeleri | Yeni tip çıkarım özellikleri |
| `vitest` | ^3.2.4 | ^3.x (en son) | Test performansı ve yeni özellikler | In-source testing, workspace desteği |
| `@playwright/test` | ^1.62.1 | ^1.x (en son) | E2E test güncellemeleri | Yeni browser özellikleri, trace viewer iyileştirmeleri |

### 2.3 İsteğe Bağlı Güncellemeler

| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `openapi-typescript` | ^7.9.1 | ^7.x (en son) | OpenAPI tip üretimi | OpenAPI 3.1 desteği |
| `@types/node` | ^22.15.3 | ^22.x (en son) | Node.js tip tanımları | Yeni Node.js API'leri |
| `@types/react` | ^19.1.8 | ^19.x (en son) | React tip tanımları | React 19 tip iyileştirmeleri |
...
| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `pydantic` | 2.13.4 | 2.x (en son) | **Güvenlik ve performans kritik** | Type coercion güvenlik düzeltmeleri, validation performansı |
| `fastapi` | 0.121.2 | 0.x (en son) | Güvenlik ve performans güncellemeleri | Request validation iyileştirmeleri, async performans |
| `uvicorn` | 0.42.0 | 0.x (en son) | ASGI sunucu güvenliği | HTTP/2 desteği, WebSocket iyileştirmeleri |

### 3.2 Önerilen Güncellemeler

| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `redis` | 6.4.0 | 6.x (en son) | Redis 8 uyumluluğu | Yeni veri yapıları, cluster desteği iyileştirmeleri |
| `pySigma` | 1.5.0 | 1.x (en son) | Sigma kural derleme iyileştirmeleri | Yeni Sigma kural türleri, backend desteği |
| `pysigma-backend-clickhouse` | 1.1.1 | 1.x (en son) | ClickHouse sorgu üretimi | Yeni ClickHouse fonksiyonları desteği |
| `PyYAML` | 6.0.3 | 6.x (en son) | YAML parsing güvenliği | YAML 1.2 uyumluluk |

### 3.3 drain3 Özel Durumu

| Paket | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|-------|--------------|----------------|---------|-------------------|
| `drain3` | git+SHA (0fb6d6e) | git+SHA (en son master) | Log template mining | typing düzeltmeleri, Python 3.13 uyumluluğu |
...
|------|--------------|----------------|---------|-------------------|
| `clickhouse/clickhouse-server` | 26.7 | 26.x (en son) | **Full-text text index GA desteği** (F1 §6.1) | Performans iyileştirmeleri, yeni index türleri |
| `quay.io/keycloak/keycloak` | 26.7.1 | 26.x (en son) | **OIDC güvenlik güncellemeleri** | Token validation iyileştirmeleri, new authentication flows |
| `redis` | 8-alpine | 8-alpine (en son tag) | Oturum ve Drain3 durumu için uyumluluk | Yeni komutlar, memory optimizasyonları |
| `postgres` | 18-alpine | 18-alpine (en son tag) | Kontrol düzlemi veritabanı | Yeni index türleri, JSONB iyileştirmeleri |

### 4.2 Önerilen Güncellemeler

| İmaj | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|------|--------------|----------------|---------|-------------------|
| `otel/opentelemetry-collector-contrib` | 0.158.0 | 0.x (en son) | OTel protokol güncellemeleri | Yeni receivers/processors, OTLP 1.0 uyumluluğu |
| `python` | 3.13-slim | 3.13-slim (en son tag) | Sidecar Python sürümü | Performans iyileştirmeleri, new stdlib özellikler |
| `alpine` | 3.22 | 3.x (en son) | Init konteynerleri | Güvenlik güncellemeleri |

### 4.3 Özel Durum: RustFS

| İmaj | Mevcut Sürüm | Önerilen Sürüm | Gerekçe | Son Değişiklikler |
|------|--------------|----------------|---------|-------------------|
| `rustfs/rustfs` | 1.0.0-rc.1 | 1.0.0 (stable bekleniyor) | S3 API uyumluluğu | Dağıtık mod "under testing" - tek düğüm kullanılıyor |
