...
- **Kaybın tespit edilmiş olması kurtarma kaynağını korumuyor.**
  ...
  veriyor; `State` sorguya hiç girmiyor. Önce doğrulanıp sonra kaybolan bir
  nesnenin WAL segmenti hâlâ silinebilir durumda.
- **Tespit hızı kurtarma penceresinden bağımsız.** 6 saatte 20 nesne, 48 saatte
  160 nesne — ~64 MB'lık nesnelerle ~10 GB. Arşiv bundan büyükse tam tarama
  penceresinden uzun sürüyor ve kayıp, segment silindikten *sonra* fark
  ediliyor. Üç sayı birbirinden habersiz seçilmiş.
