  ...
  async function submitReview(state: string) {
    ...
    try {
      const saved = (await api.post("/v1/rca/{id}/review", {
        ...
        body: { state, actual_root_cause: rootCause, note: "" },
      })) as RcaReview;
  ...
  return (
    <article className={styles.report}>
      ...
      <section className={styles.reviewBox} aria-labelledby="inceleme">
        ...
        {review ? (
          <p className={styles.reviewSaved} data-review-state={review.state}>
            Son inceleme: <strong>{REVIEW_STATES.find((s) => s.value === review.state)?.label ?? review.state}</strong>
            {" — "}
